...
def notebooks_data():
    with open('RES.csv', encoding='utf-8') as data:
...
def notebook_edit(notebooks, index):
    name = input("\
██████████████████████████████████████████\n\
█ Введи новое имя для своего блокнота:   █\n\
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\n\
█ ==> \
    ")
...
def notebook_add(notebooks, move):
    name = input("\
██████████████████████████████████████████\n\
█ Введи имя для своего нового блокнота:  █\n\
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\n\
█ ==> \
")
    ...
    del pathname #удаление pathname
...
def schedule_write(schedule, notebook):
    if notebook["Path"] == "RES.csv": path = "RES.csv"
...
def notebook_action(schedule):
    if len(notebooks) == 1:
        move = list(input("\
█ Нажмите + для создания нового блокнота:  █\n\
█ Нажмите L для завершения работы:         █\n\
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\n\
█ ==> \
").split(' '))
        move.append('~')
        ...
        move = list(input("\
█ Выбери действие:                   █\n\
█ O ══ Открыть блокнот <Номер>       █\n\
█ + ══ Создать блокнот               █\n\
█ ~ ══ Переименовать блокнот <Номер> █\n\
█ - ══ Удалить блокнот <Номер>       █\n\
█ L ══ Завершить работу              █\n\
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\n\
█ ==> \
").split(' '))
...
def schedule_action(schedule):
    if len(schedule) == 1:
            path = list(input("\
█ Нажмите + для создания нового блокнота: █\n\
█ Нажмите C для закрытия блокнота:        █\n\
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\n\
█ ==> \
").split(' '))
...
█ Выберите действие:                 █\n\
█ + ══ Добавить текст                █\n\
█ ~ ══ Изменить <номер блокнота>     █\n\
█ - ══ Удалить <номер блокнота>      █\n\
█ ! ══ Сортировать                   █\n\
█ D ══ Очистить блокнот              █\n\
█ C ══ Закрыть блокнот               █\n\
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\n\
...
# -------------------
    ...
    notebooks.pop(0)
    schedule_write(notebooks, {"Path" : "RES.csv"})
